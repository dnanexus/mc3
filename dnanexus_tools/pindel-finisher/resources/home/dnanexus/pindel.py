...
def __main__():
    ...
    try:
        ...
        newInputFiles = []
        i = 0
        inputFastaLink = os.path.join(os.path.abspath(args.workdir), "reference.fa" )
        if not os.path.exists(inputFastaLink):
            os.symlink(args.inputFastaFile, inputFastaLink)
            cmd = "samtools faidx %s" %(inputFastaLink)
            execute(cmd)    
        inputFastaFile=inputFastaLink
        """
        ...
        if args.outputRaw is not None:
            shutil.copy(args.outputRaw, os.path.join(args.workdir, "pindel_all"))
        ...
        if args.outputSomaticVcfFile is not None:
            with open(os.path.join(args.workdir, "pindel_somatic"), "w") as handle:
                with open(os.path.join(args.workdir, "pindel_all")) as pindel_file:
                   for line in pindel_file:
                        if re.search("ChrID", line):
                            svtype=line.strip().split()[1]
                            if svtype.startswith("D") or svtype.startswith("I"):
                                handle.write(line)
